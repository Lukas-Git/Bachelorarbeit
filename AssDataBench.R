...
t284 <- fread("D:/Daten/284_Table_bench.csv",quote = "\"")
...
while(i<= nrow(t284new))
{
  ...
  else
  {
    ...
    {
      name <- names(t284[,..j])
     t284new[[i,j]] <- (as.numeric(t284proxy[[i,j]])/ as.numeric(t284proxy[[(i-3),j]])) - 1
      j <- j+1
    }
  }
  i <- i + 1
...
data.table::fwrite(t284new, "D:/Daten/asmblglvl2/AssDataBench.csv")
