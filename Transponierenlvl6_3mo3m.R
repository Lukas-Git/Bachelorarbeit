...
while(i<= nrow(t284new))
{
  ...
  else
  {
    ...
    {
      name <- names(t284[,..j])
      t284new[[i,j]] <- (((as.numeric(t284proxy[[i,j]]))/ (as.numeric(t284proxy[[i-1,j]])) + (as.numeric(t284proxy[[i-1,j]]))/ (as.numeric(t284proxy[[i-2,j]]))+(as.numeric(t284proxy[[i-2,j]]))/ (as.numeric(t284proxy[[i-3,j]])))/3  - 1)*100
      j <- j+1
    }
  }
  i <- i + 1
...
data.table::fwrite(t284new, "D:/Daten/asmblglvl2/AssDataxlvl6xtra2.csv")
